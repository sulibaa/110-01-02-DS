...
///////////////////////////////////////////////////////////////////////////////////////////////// mission1_print
  	...
  	void mission2(){
		...
		if(changing_list[listSize-1].OID == changing_list[listSize-2].OID ) listSize--; // 有些input會多一個"\n"
		 
		now_list = changing_list[0]; // 抓第一筆資料
		cooking_times = changing_list[0].Arrival + changing_list[0].Duration; // 第一筆完成時間
		now++; 
	    update_QueueSize();
		while( now < listSize ||  QueueSize > 0 || now_list.OID != 0) { 
	      if ( beforeCooking() ) { // 處理訂單前檢查是否超時
		    Cooking();
			InsertQueue(); 
			update_QueueSize();
	      } // if()
